...
| 🗑️ 삭제 기능 | [`admin.sql`](admin.sql) | 운영자 비번 검증 삭제 RPC (선택, SQL 1회 실행) |
...
익명(anon) 사용자는 **읽기 + 등록만** 가능. 수정/삭제는 RLS로 막혀 있습니다.

### 운영자 삭제 기능 (선택)
운영자 화면(`admin.html`)에서 글·신청을 지우려면 [`admin.sql`](admin.sql)을 한 번 실행하세요:
1. `admin.sql`의 `CHANGE_ME_바꾸세요`(3곳)를 원하는 **운영자 비밀번호**로 바꾸기
2. Supabase → SQL Editor에 붙여넣고 Run
3. `admin.html`에서 🗑 삭제 시 그 비밀번호 입력 (이 기기에만 저장됨)

> 삭제는 anon 키가 아니라 **서버에서 비번을 검증하는 함수(RPC)**로만 동작 → 공개 키로는 아무도 데이터를 못 지웁니다. 실제 비밀번호는 레포에 커밋하지 마세요.
