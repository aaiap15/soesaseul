# ⛓️ 쇠사슬 — 크루로 운동 지속 (수요 검증 MVP)

"같이 운동하자"는 약속이 흐지부지되지 않게, **크루의 공동 스트릭(쇠사슬)**으로 서로 끌고 가는 앱.
...
| 역할 | 파일 | 설명 |
|---|---|---|
| 📱 크루 앱 | [`index.html`](index.html) | 크루 생성·코드 참여·10초 인증·쇠사슬·보드·치킨판·원격 타이머·가디언 |
...
| 🔌 데이터·로직 | [`db.js`](db.js) | Supabase↔localStorage 자동 전환 + `computeChain` 쇠사슬 계산(공용) |

## 핵심 장치 (사업계획 §4)
- **쇠사슬(공동 스트릭)** — 인증마다 고리 +1, 주간 미달 시 절단, 최고 기록 보존
...
- 익명 사용자는 읽기+등록만 (RLS). 쇠사슬 계산은 인증 이력에서 `computeChain`이 파생.
